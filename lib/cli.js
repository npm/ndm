...
Cli.prototype.remove = function(serviceName) {
  ...
  this.stop(serviceName, function() {
    ...
    _this.service.allServices().forEach(function(service) {
...
Cli.prototype._runCommand = function(command, serviceName, cb) {
  ...
  async.each(this.service.allServices(), function(service, done) {
    // if we provide a serviceName parameter, only run command
    // on the specific service.
    if (serviceName && service.name !== serviceName) return done();
...
Cli.prototype.install = function() {
  var _this = this,
    serviceJsonPath = temp.path({suffix: '.json'}),
    interview = new this.Interview({
      tmpServiceJsonPath: serviceJsonPath
    });

  interview.run(function() {
    config.serviceJsonPath = serviceJsonPath; // point to temporary config.
    Config(config); // write the changes.
    _this.generate(); // generate the service wrappers.
    _this.rimraf.sync(serviceJsonPath); // remove the temporary service.json.
  });
};
