...
Service.prototype._workingDirectory = function () {
  const packageJson = this.utils.loadServiceJson(path.resolve(
    this.baseWorkingDirectory,
    './package.json'
  ))
  const module = this.module || this.name
...
Service.prototype._getPlatformApi = function () {
  const API = this.platformApis[this.platform] || this.platformApis.ubuntu
  return new API()
}
...
Service.prototype.generateScript = function (cb) {
  ...
  if (this.runnable()) {
    ...
    const flatArgs = _.isArray(this.args) ? this.args : this._flattenArgs()
...
Service.prototype._startScript = function (runArgs) {
  let startScript = this.scripts.start.replace(/^node +/, '')
  const splitScript = startScript.split(/[ =]/)
...
Service.prototype._copyFieldsFromPackageJson = function () {
  const _this = this
  const fieldsToCopy = ['scripts', 'description']
  let packageJson = null
  const packageJsonPath = path.resolve(path.dirname(this.serviceJsonPath), 'package.json')
...
exports.allServices = function (serviceNameFilter) {
  ...
  const serviceJsonPath = exports._serviceJsonPath(serviceNameFilter)
...
exports.transformPackageJson = function (packageJson) {
  const serviceJson = {}
  const innerJson = serviceJson[packageJson.name] = {}
...
function parseServiceJson (serviceNameFilter, serviceJson) {
  ...
  Object.keys(serviceJson).forEach(function (serviceName) {
    ...
    const serviceConfig = serviceJson[serviceName]
    const processCount = serviceConfig.processes || 1
    ...
    _.range(processCount).forEach(function (i) {
      ...
      // the services.
      const service = new Service(_.extend(
        {
          module: serviceName,
...
function dropInterviewQuestions (o) {
  return _.reduce(o, function (result, v, k) {
    const value = typeof v === 'object' ? v.default : v
...
function expandVariables (service, processNumber) {
  ...
  service.env = _.reduce(service.env, function (result, v, k) {
    const value = typeof v === 'string' ? v.replace(/%i/g, processNumber) : v
    return (result[k] = value, result)
  }, {})
...
exports._serviceJsonPath = function (serviceNameFilter) {
  const config = require('./config')()
  var packageJsonPath = path.resolve(path.dirname(config.serviceJsonPath), './package.json')
  var baseWorkingDirectory = null
  const pathsToCheck = [
  ...
  if (baseWorkingDirectory) {
    ...
    const serviceJsonPath = path.resolve(baseWorkingDirectory, 'service.json')
    var packageJsonPath = path.resolve(baseWorkingDirectory, 'package.json')
