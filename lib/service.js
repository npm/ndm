...
Service.prototype.startCommand = function() {
  ...
  return 'sudo service ' + this.name + ' start';
};

Service.prototype.restartCommand = function() {
  if (this.platform === 'centos') {
  ...
  return 'sudo service ' + this.name + ' restart';
};

Service.prototype.stopCommand = function() {
  if (this.platform === 'centos') {
    return 'sudo initctl stop' + this.name;
  } else if (this.platform === 'darwin') {
    return 'launchctl unload ' + this.scriptPath();
  }

  return 'sudo service ' + this.name + ' stop';
};
