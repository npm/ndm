...
exports.allServices = function(serviceNameFilter) {
  ...
  if (serviceJsonPath.indexOf('package.json') !== -1) return [];
  else return parseServiceJson(serviceNameFilter, serviceJson);
};

// convert a package.json into a service.json format.
function parsePackageJson(serviceNameFilter, serviceJson) {
...
exports._serviceJsonPath = function(serviceNameFilter) {
  ...
  if (fs.existsSync(config.serviceJsonPath) || !serviceNameFilter) return config.serviceJsonPath;
  ...
  if (baseWorkingDirectory) {
    ...
    config.serviceJsonPath = path.resolve(baseWorkingDirectory, 'service.json');
